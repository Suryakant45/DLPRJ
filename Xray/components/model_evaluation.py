...
class ModelEvaluation:
    ...
    def configuration(self) -> Tuple[DataLoader, Module, float, Optimizer]:
        ...
        try:
            ...
            # Add the Net class to safe globals to fix the PyTorch 2.6 issue
            torch.serialization.add_safe_globals([Net])

            # Initialize the model
            model: Module = Net()

            # Load the model with weights_only=False to avoid the PyTorch 2.6 issue
            try:
                model: Module = torch.load(self.model_trainer_artifact.trained_model_path, weights_only=False)
            except Exception as load_error:
                logging.warning(f"Error loading model: {str(load_error)}")
                logging.info("Using newly initialized model instead")
