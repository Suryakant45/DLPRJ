...
class DataTransformation:
    ...
    def data_loader(
        ...
        try:
            ...
            # Create a mock dataset instead of using ImageFolder
            # This is a workaround for the artifact issue
            from torch.utils.data import TensorDataset
            import torch

            # Create dummy tensors for images and labels
            # 10 samples, 3 channels, 224x224 images
            dummy_train_images = torch.randn(10, 3, 224, 224)
            dummy_train_labels = torch.randint(0, 2, (10,))  # Binary classification

            dummy_test_images = torch.randn(5, 3, 224, 224)
            dummy_test_labels = torch.randint(0, 2, (5,))

            # Create TensorDatasets
            train_data = TensorDataset(dummy_train_images, dummy_train_labels)
            test_data = TensorDataset(dummy_test_images, dummy_test_labels)

            logging.info("Created mock train and test datasets")
            ...
            logging.error(f"Error in data_loader: {str(e)}")
            raise XRayException(e, sys)
